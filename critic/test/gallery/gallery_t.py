...
A = ROOT.arttest.StringProduct
A_vec = ROOT.std.vector[A]
B = int
B_vec = ROOT.std.vector[B]
D = ROOT.critictest.LiteAssnTestData

AssnsABD = ROOT.art.Assns[A, B, D]
AssnsBAD = ROOT.art.Assns[B, A, D]
...
while not ev.atEnd():
  ...
  assnsABHandle1 = ROOT.gallery.Handle[AssnsABD]()
  ev.getByLabel(inputTagAssnTest1, assnsABHandle1)

  assnsABHandle2 = ROOT.gallery.Handle[AssnsABD]()
  ev.getByLabel(inputTagAssnTest2, assnsABHandle2)

  assnsBAHandle3 = ROOT.gallery.Handle[AssnsBAD]()
  ev.getByLabel(inputTagAssnTest1, assnsBAHandle3)

  assnsBAHandle4 = ROOT.gallery.Handle[AssnsBAD]()
  ...
  hVStringProduct = ROOT.gallery.Handle[A_vec]()
  ev.getByLabel(inputTag111, hVStringProduct)
  assert hVStringProduct.isValid()

  findMaker = ROOT.gallery.FindMaker()

  findOne = findMaker.makeFindOne[B, D](hVStringProduct, ev, inputTagAssnTest1)
  ...
  B_vec = ROOT.std.vector[int]
  hB = ROOT.gallery.Handle[B_vec]()
  ev.getByLabel(inputTag111, hB)
  assert hB.isValid()

  findOneBA = findMaker.makeFindOne[A, D](hB, ev, inputTagAssnTest1)
  ...
  findOneP = findMaker.makeFindOneP[B, D](hVStringProduct, ev, inputTagAssnTest1)
  ...
  findManyBA = findMaker.makeFindMany[A, D](hB, ev, inputTagAssnTest1)
  ...
  findManyP = findMaker.makeFindManyP[B, D](hVStringProduct, ev, inputTagAssnTest1)
